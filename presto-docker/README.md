...
```
docker pull presto-book/docker-presto:latest
```

## Build
```
docker build --rm -t presto-book/docker-presto:latest .
```
...
```
docker run -p 8080:8080 presto-book/docker-presto:latest
```
...
Example:
```
docker run -e PRESTO_CATALOG_HIVE=true -it presto-book/docker-presto
```
